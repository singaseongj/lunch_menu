...
- 🍽️ 선택한 날짜의 아침, 점심, 저녁 메뉴와 열량 정보를 표 형태로 깔끔하게 보여 줍니다.
- 🔄 급식 데이터가 갱신된 시각을 한국어 타임스탬프로 노출해 최신 여부를 쉽게 확인할 수 있습니다.
- 🛡️ 급식 데이터가 비어 있거나 불러오기에 실패한 경우를 감지해 사용자에게 안내 메시지를 제공합니다.
- ⭐ 각 메뉴에 1~5점 별점 투표 버튼을 제공하고, 별도 결과 페이지에서 메뉴별 투표 결과를 확인할 수 있습니다.

## 동작 방식

프런트엔드 스크립트(`menu.js`)는 기본적으로 `data/menu-data.json` 파일을 불러와서 화면을 구성합니다. 파일에는 사용 가능한 날짜 목록, 각 급식 시간대별 메뉴(`items`)와 열량(`calories`), 생성 시각이 포함되어 있으며, 초기화 시 오늘 날짜 또는 가장 가까운 급식 데이터를 선택하여 렌더링합니다. 메뉴별 별점 투표는 Google Apps Script 웹 앱으로 `{ "name": "메뉴 이름", "rating": 1~5 }` JSON을 POST로 전송하고, `vote-results.html`은 같은 웹 앱의 `{ ok, data: [{ name, averageRating, voteCount }] }` 응답을 읽어 표로 보여 줍니다.
