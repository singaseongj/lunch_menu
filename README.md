...
프런트엔드 스크립트(`menu.js`)는 기본적으로 `data/menu-data.json` 파일을 불러와서 화면을 구성합니다. 파일에는 사용 가능한 날짜 목록, 각 급식 시간대별 메뉴(`items`)와 열량(`calories`), 생성 시각이 포함되어 있으며, 초기화 시 오늘 날짜 또는 가장 가까운 급식 데이터를 선택하여 렌더링합니다.
